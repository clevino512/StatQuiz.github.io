...
// Fonction de connexion (version JS uniquement, sans backend)
async function loginUser() {
    const username = document.getElementById('username').value;
    const password = document.getElementById('password').value;

    // Utilisateur(s) fictif(s) pour la démo
    const users = [
        { username: "admin", password: "123456", userId: 1 },
        { username: "user", password: "password", userId: 2 }
    ];
    ...
    if (user) {
        // Stockage d'un token fictif et redirection
        localStorage.setItem('quizToken', 'fake-token-' + user.userId);
        localStorage.setItem('quizUserId', user.userId);
...
// Appeler checkAuth() au chargement des pages protégées
// Ajoutez ceci au début de index.html dans une balise <script>
